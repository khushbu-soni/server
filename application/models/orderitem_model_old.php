...
class orderitem_model extends CI_Model{
    ...
    public function table_bill_amount($tablenumber){
        ...
        $query=$this->db->query("SELECT SUM(price) as bill_amount FROM `table` join orderitem on orderitem.table_no=`table`.tablenumber WHERE orderitem.table_no='".$tablenumber."' AND `table`.inuse=1"); 
