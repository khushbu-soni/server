...
class Menu extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data['w_id']=$this->session->userdata('waiter_id');
		$this->data['menuheader'] = $this->load->view('waiter/customermenuheader', '', TRUE);
		$this->data['dependencies'] = $this->load->view('general/dependencies', '', TRUE);
		$this->data['yourorder'] = $this->load->view('waiter/yourorder', '', TRUE);
		
		$this->data['moreinfo'] = $this->load->view('waiter/moreinfo', '', TRUE);
		$this->data['customize'] = $this->load->view('waiter/customize', '', TRUE);
	...
	public function index()
	{
		...
		$this->data['menucontent'] = $this->load->view('waiter/menucontent', $content_data, TRUE);
		...
		$this->load->view('waiter/menu', $this->data);
	}
		...
		$temp_data_q=$quantitys->row();	
		 ...
		 <button price='$temp_data->price' quantity='$tempitem->quantity' itemid='$tempitem->itemid' onclick='quantityAdd(event)' class=bootstrap-touchspin-down type=button>+</button>";
  		...
  		if($tempitem->quantity != 1)
            ...
            <a href='#' itemprice='$tempitem->price' itemid='$tempitem->itemid' onclick='removeFromOrder(event)' class='btn'><i class='icon-remove'></i> Remove</a></div>";
	...
	$tempitems_html .= "</div>";
	$tempitems_html .= "<div class=pull-right >
			<span style=font-weight:bold;>Total: $</span>
			<span id=ordertotal>$total</span></div>";
		...
		$temp_data=$quantitys->row();	
          ...
          // $tempitems_html .= "<b> $tempitem->itemname</b>";
            ...
            <br>Quantity: <b> <input style='color:red; width:20px;' id=demo3 value=$tempitem->quantity type=text name=demo_vertical2>
             ...
             <button price_og='$menuid->price' price='$temp_data->price' quantity='$tempitem->quantity' itemid='$tempitem->itemid'onclick='quantityMinus(event)' class=bootstrap-touchspin-down type=button>-</button>
            </b></span><br/><br/>
            <a href='#' itemprice='$tempitem->price' itemid='$tempitem->itemid' onclick='removeFromOrder(event)' class='btn'><i class='icon-remove'></i> Remove</a></div>";
            $total = $total + $tempitem->price; 
         }
	...
	/*$quantitys = $this->db->query("select * from temp_your_order where itemid=$menuitem->id");
		...
		$temp_data=$quantitys->row();
			...
			if ($index % 2 == 0){
				...
				$menuitems_html .= "<span class='' ><b>" . $menuitem->name . "</b></span><br><span>&nbsp;<b class='menu-price'>$ " . $menuitem->price . "</b></span><br/><br/><a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
				...
				$menuitems_html .= "<span>" . $menuitem->name . "</span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$ " . $menuitem->price . "</b></span><br/><br/>
	...
	/*$quantitys = $this->db->query("select * from temp_your_order where itemid=$menuitem->id");
		...
		$temp_data=$quantitys->row();
			...
			if ($index % 2 == 0){
				...
				$menuitems_html .= "<span>" . $menuitem->name . "</span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$ " . $menuitem->price . "</b></span><br/><br/><a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
		...
		if (empty($menuitems)){
			...
			foreach ($menuitems as $menuitem){
				if ($index % 2 == 0){
					...
					<span>" . $menuitem->name . "</span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$ " . $menuitem->price . "</b></span><br/><br/>
					<a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
	...
	public function print_placeorder_kot()
	 {
	     	...
	     	$this->load->model('orderitem_model');
          	...
          	$this->data['tempitems'] = $this->orderitem_model->get_tempitem_custname();
          	$this->data['customername'] = $this->session->userdata('customername');
          	$this->data['tablenumber'] = $this->session->userdata('tablenumber');
          	$this->data['waiter_id'] = $this->session->userdata('waiter_id');
          ...
          {
          	$this->data['orderid'] = $this->session->userdata('order_id');	
          }
          
          	$this->load->view('waiter/kot_print', $this->data);
