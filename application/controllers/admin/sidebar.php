...
class sidebar extends CI_Controller {
	...
	public function sidebarMENU()
	{
		...
		$sidebars = $this->configruation_model->sidebar_menus("Admin");
                        ...
                        if(!empty($record->childs)) {
                        	...
                        	foreach($record->childs as $c) 
                        	{
                                  ...
                                  echo '<ul class="nav nav-second-level">
				                            <li>
				                               <a class=""  href="'.site_url().'admin/'.$c->menu_url.'"><i class="fa '.$record->icon.' fa-3x"></i>'.$c->menu_name.'</a>
				                            </li>
                            ...
                            echo  '<a class=""  href="'.base_url().'admin/'.$record->menu_url.'"><i class="fa '.$record->icon.' fa-3x"></i>'.$record->menu_name.'</a>';
                        }
                         echo '</li>';
...
function rederTreeByIdRes($records, $id=false) {
	 ...
	 echo '<select onchange="location = this.options[this.selectedIndex].value;"class="form-control" width="50px;">
        ...
        foreach($records as $record) {
                       ...
                       echo  '<option value='.site_url().'admin/'.$record->menu_url.'>'.$record->menu_name.'</option>';
                        ...
                        if(!empty($record->childs)) {
                        	...
                        	foreach($record->childs as $c) 
                        	{
				                 ...
				                 echo ' <option value='.site_url().'admin/'    .$c->menu_url.'>'.$c->menu_name.'</option>';  
