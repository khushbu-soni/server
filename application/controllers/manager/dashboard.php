...
class Dashboard extends CI_Controller {
 ...
 $drop="<label for='exampleInputName2'>Table No</label>
	...
	public function get_item_list(){
		...
		//echo $item_no_str;
		$item_no_strs=trim($item_no_str,"");
		$item_no_array=explode(',',$item_no_strs);

		//print_r($item_no_array);
		$item_no=array();

		foreach ($item_no_array as $key => $value) {
			if($value!=""){
				//echo "string";
				$item_no[$key]=$value;
			}
		...
		$strs=implode(',',$item_no);
		//echo "<br>".trim($strs,",");
		$str=trim($strs,",");
		$html ='<table class="table">
				';
		$html.="<input type='hidden' value='".$order_id."' name='order_id' id='order_id'/>";
